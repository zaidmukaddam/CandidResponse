...
export default function Dialogue() {
    ...
    const responseParsed = response.split(/\r?\n/).map((line, index) => {
        if (index > 1) return (
            <p className={style["candidresponse-text"]}
                key={index} >
                {line}
            </p>
        )
    ...
    // activates an active state for candidresponse Logo if the user has stopped tying for 6 seconds.
    ...
    return (
        <div className={style["dialogue-wrapper"]} id={isLoaded ? style["dialogue-wrapper-loaded"] : ''}>
            ...
            <div className={style["candidresponse-wrapper"]}>
                <svg className={style["candidresponse-input-logo"]}
